...
public class NumberTheoreticTransform {
     ...
     */
    public NumberTheoreticTransform(BigInteger[] coefficients, BigInteger polynomialDegree, BigInteger modulus) {
    ...
    private void initializeContext() {
        ...
        for (int i = 0; i < polynomialDegree.intValue(); i++) {
            powersOfRootOfUnity[i] = NumberTheory.raiseExponentInModulus(this.rootOfUnity, BigInteger.valueOf(i), this.modulus);
            inversePowersOfRootsOfUnity[i] = NumberTheory.raiseExponentInModulus(inverseRootOfUnity, BigInteger.valueOf(i), this.modulus);
            reversedBits[i] = NumberTheory.modInverseWithPrimeModulus(BitOperations.bitReversal(BigInteger.valueOf(i), width), modulus);
        }
